...
namespace CapaDatos
{
    public class CD_ComprobantesCES
    {
        public List<ComprobantesCES> Listar()
        {
            List<ComprobantesCES> ComprobantesCESLista = new List<ComprobantesCES>();
            string Mensaje = string.Empty;
            try
            {
                using (SqlConnection ComprobantesCESConexion = new SqlConnection(Conexion.cn))
                {

                    StringBuilder sb = new StringBuilder();
                    sb.AppendLine("SELECT [Fecha de Operacion],[Tipo Documento Identidad Asegurado Titular] ,[Número Documento Identidad Asegurado Titular] ,[Nombres / Razon Social Asegurado Titular] ,[Apellido Paterno Asegurado Titular] ,[Apellido Materno Asegurado Titular] ,[Ubigeo del Cliente] ,[Prima a Facturar] ,[Número de Póliza] ,[Inicio Vigencia] ,[Fin  Cobertura (openItem)] ,[Suma Asegurad] ,[Nombre_Producto] ,[Mes] FROM[dbo].[COMPROBANTES_CES]");
                    SqlCommand cmd = new SqlCommand(sb.ToString(), ComprobantesCESConexion);
                    cmd.CommandType = CommandType.Text;
                    ComprobantesCESConexion.Open();
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            ComprobantesCESLista.Add(
                                new ComprobantesCES()
                                {
                                    ...
                                    Suma_Asegurad = Convert.ToString(dr["Suma Asegurad"]),
            ...
            {
                ComprobantesCESLista = new List<ComprobantesCES>();
            }
            return ComprobantesCESLista;
        }
        public int Exportar(Producto obj, string Year, string Mes, out string Mensaje)
        {
            ...
            try
            {
                using (SqlConnection ComprobantesCESConexion = new SqlConnection(Conexion.cn))
                {
                    SqlCommand cmd = new SqlCommand("SP_ExportarCES", ComprobantesCESConexion);
